...
resource "aws_db_instance" "rds_instance" {
  ...
  instance_class        = each.key == "mediator" || each.key == "credo-controller" ? var.aries_db : var.platform_db
  engine                = "postgres"
  engine_version        = data.aws_rds_engine_version.latest_postgres.version
  allocated_storage     = var.environment == "dev" ? 30 : (each.key == "mediator" || each.key == "credo-controller" ? 100 : 50)
  storage_type          = var.environment == "dev" ? "gp2" : (each.key == "mediator" || each.key == "credo-controller" ? "io2" : "gp2")
  max_allocated_storage = 500
  # Set IOPS conditionally based on storage type
  iops                   = var.environment == "dev" ? "gp2" : (each.key == "mediator" || each.key == "credo-controller" ? each.value.iops : null)

  multi_az               = true
  username               = lower(replace(each.value.username, "-", ""))
